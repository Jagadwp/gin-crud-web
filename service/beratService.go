...
func (b *BeratService) GetAllBerat() (*[]model.Berat, error) {
	fmt.Println("MASUKKK Service")
	...
	var data []model.Berat

	for _, berat := range *berats {
		tempData := model.Berat{
			ID:        berat.ID,
			Min:       berat.Min,
			Max:       berat.Max,
			Diff:      berat.Diff,
			Date:      berat.Date,
			CreatedAt: berat.CreatedAt,
			UpdatedAt: berat.UpdatedAt,
		}
