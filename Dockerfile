...
RUN env GOOS=linux GOARCH=amd64 \
    go build -o bin/gin-crud-web -v app/main.go
...
COPY --from=builder /build/.env /build/bin/gin-crud-web ./
...
# docker run -p 8080:8080 -it --rm gin-crud-web:1.0
